...
- instead of taking mu and nu as constant, the program takes mu as constant and tau is a param in model
...
class MonLipNet(nn.Module):
    ...
    nu_initial_guess: jnp.float32 = 25.
    ...
    def __call__(self, x : jnp.array) -> jnp.array:
        ...
        logtau = self.param('logtau', nn.initializers.constant(jnp.log(self.nu_initial_guess/self.mu)), (1,), jnp.float32)
...
class BiLipNet(nn.Module):
    ...
    # nu: float = 10.
    depth: int = 2
    nu_initial_guess: jnp.float32 = 16

    def setup(self):
        uni, mon = [], []
        mu = self.mu ** (1. / self.depth)
        nu_initial_guess = self.nu_initial_guess ** (1. / self.depth)
        # nu = self.nu ** (1. / self.depth)
        for _ in range(self.depth):
            uni.append(Unitary())
            mon.append(MonLipNet(self.units, mu=mu, nu_initial_guess=nu_initial_guess))
        uni.append(Unitary())
        self.uni = uni
        self.mon = mon
